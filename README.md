...
## Quick Docker Start
  ...
  ```bash
    ...
    source ~/.venvs/beautiful_geonode/bin/activate
...
**NOTE**: *You can call your geonode project whatever you like **except 'geonode'**. Follow the naming conventions for python packages (generally lower case with underscores (``_``). In the examples below, replace ``beautiful_geonode`` with whatever you would like to name your project.*
...
1. Generate the project
    ...
    mkvirtualenv --python=/usr/bin/python3 beautiful_geonode
    pip install Django==3.2.16

    django-admin startproject --template=./geonode-project -e py,sh,md,rst,json,yml,ini,env,sample,properties -n monitoring-cron -n Dockerfile beautiful_geonode

    cd beautiful_geonode
    ```
...
```bash
SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./beautiful_geonode/br/backup.sh $BKP_FOLDER_NAME
```
...
```bash
docker exec -it django4beautiful_geonode sh -c 'SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./beautiful_geonode/br/backup.sh $BKP_FOLDER_NAME'
```

### Run a Restore

```bash
SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./beautiful_geonode/br/restore.sh $BKP_FOLDER_NAME
```
...
```bash
docker exec -it django4beautiful_geonode sh -c 'SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./beautiful_geonode/br/restore.sh $BKP_FOLDER_NAME'
```
